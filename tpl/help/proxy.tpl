...
<h1><a class="h" href="##selfhelp_list##" title="Back to help overview">DiCoP</a> - Proxy</h1>
...
<ul>
	...
	<li><a href="#proxy__client_connect">PROXY - CLIENT CONNECT</a></li>
	<li><a href="#different_request_types">DIFFERENT REQUEST TYPES</a></li>
	<ul>

		<li><a href="#administrative_messages">ADMINISTRATIVE MESSAGES</a></li>
	...
	<ul>
		...
		<li><a href="#maximum_possible_clients">Maximum possible clients</a></li>
		<li><a href="#network_performance">Network performance</a></li>
	</ul>
...
<p>In addition, here is the picture with file servers included:</p>
...
<p>Note that file servers can either run at the same machine than the server, or
the proxy, or on a totally different machine.</p>

<p>In this case we focus on the proxy part. A <code>Dicop::Proxy</code> works almost
like a <code>Dicop::Server</code>, with a few notable exceptions:</p>
...
<li>The proxy does not read in data, and also never writes out data. It simple
relays client connections to the main server.



<li>The proxy answers file requests with it's own file server settings.
...
<p>Clients behind the proxy can only request work, testcases and files and
submit their results. The can never view any status pages at the server,
nor is it possible to administrate the server via the proxy.</p>
...
server is not neccessary at all.</p>
...
<p>XXX TODO: This section does not apply yet.</p>
...
<p>The proxy knowns only the requests it can handle, this is for security reasons.</p>

<p>All other requests will be denied as ``invalid request''.</p>
...
<p>Fewer connects with more reqests per connect assume that there is an overhead
per connect that can be saved.</p>
...
<p>The proxy should be able to handle roughly the same number of clients as
